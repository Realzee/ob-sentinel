...
export default function UserManagementModal({ open, onClose, currentUserRole = 'user', currentUserCompanyId }: UserManagementModalProps) {
  ...
  const loadUsers = async () => {
    try {
      ...
      if (usersData && Array.isArray(usersData)) {
        setUsers(usersData);
        console.log('✅ Loaded users via API:', usersData.length);
      } else {
  ...
  const handleRoleUpdate = async (userId: string, newRole: string) => {
    ...
    try {
      ...
      showError('Error updating user role: ' + (error.message || 'Please try again.'));
    } finally {
      setUpdating(null);
    }
  };
  ...
  const handleStatusUpdate = async (userId: string, newStatus: string) => {
    ...
    try {
      ...
      showError('Error updating user status: ' + (error.message || 'Please try again.'));
    } finally {
      setUpdating(null);
    }
  };
    ...
    showConfirmation(
      ...
      async () => {
        try {
          ...
          showError('Error suspending user: ' + (error.message || 'Please try again.'));
        } finally {
          setUpdating(null);
        }
      }
  ...
  const handleApproveUser = async (userId: string) => {
    try {
      ...
      showError('Error approving user: ' + (error.message || 'Please try again.'));
    } finally {
      setUpdating(null);
    }
  };
    ...
    showConfirmation(
      ...
      async () => {
        try {
          ...
          showError('Error reactivating user: ' + (error.message || 'Please try again.'));
        } finally {
          setUpdating(null);
        }
      }
    ...
    showConfirmation(
      ...
      async () => {
        try {
          ...
          showError('Error deleting user: ' + (error.message || 'Please try again.'));
        } finally {
          setUpdating(null);
        }
      }
  ...
  const handleBulkRoleUpdate = async (newRole: string) => {
    ...
    try {
      ...
      showError('Error updating users: ' + (error.message || 'Please try again.'));
    }
  };
    ...
    showConfirmation(
      ...
      async () => {
        try {
          ...
          showError('Error deleting company: ' + (error.message || 'Please try again.'));
        } finally {
          setUpdating(null);
        }
      }
          ...
          <div className="relative inline-block w-full max-w-6xl px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-gray-900 rounded-2xl border border-gray-700 shadow-2xl sm:my-8 sm:align-middle sm:p-6">
            ...
            <div className="mb-6 border-b border-gray-700">
              <div className="flex space-x-8">
                ...
                >
                  Company Management {!isAdminUser && `(Admin Only - Your role: ${detectedUserRole})`}
                </button>
              </div>
            </div>
